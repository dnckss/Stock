...
import {
  AlertTriangle, ShieldCheck, FileText, TrendingDown, Activity,
...
const ALLOC_COLORS = [
  '#14b8a6', '#38bdf8', '#84cc16', '#f59e0b', '#f97316',
  '#ef4444', '#a78bfa', '#64748b', '#22c55e', '#06b6d4',
];
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-zinc-950/40 border border-zinc-800/70 rounded-xl overflow-hidden ${className}`}>
      {children}
    </div>
  );
}
...
function AllocTooltip({ active, payload }: { active?: boolean; payload?: AllocTooltipPayload[] }) {
  ...
  if (!d) return null;
  return (
    <div className="rounded-lg border border-zinc-700/70 bg-zinc-950/95 px-3 py-2 shadow-xl backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 mt-1">
        ...
        <span className="text-xs font-mono text-emerald-300 font-bold">{d.weightPct.toFixed(1)}%</span>
      </div>
    </div>
  );
}
...
function SectorTooltip({ active, payload }: { active?: boolean; payload?: SectorTooltipPayload[] }) {
  ...
  if (!d) return null;
  return (
    <div className="rounded-lg border border-zinc-700/70 bg-zinc-950/95 px-3 py-2 shadow-xl backdrop-blur-lg text-xs font-mono">
      <span className="text-zinc-300">{d.sector}</span>
      <span className="ml-2 text-emerald-300 font-bold">{d.pct}%</span>
    </div>
  );
}
...
function ScenarioTooltip({ active, payload }: { active?: boolean; payload?: ScenarioTooltipPayload[] }) {
  ...
  if (!d) return null;
  return (
    <div className="rounded-lg border border-zinc-700/70 bg-zinc-950/95 px-3 py-2 shadow-xl backdrop-blur-lg max-w-[240px]">
  ...
  return (
    <div className="rounded-lg border border-zinc-700/70 bg-zinc-950/95 px-3 py-2 shadow-xl backdrop-blur-lg text-xs font-mono">
      ...
      <div className="space-y-0.5">
        <p><span className="text-zinc-500">Annual Vol: </span><span className="text-sky-300">{d.vol.toFixed(2)}%</span></p>
      ...
      <CardHeader
        ...
        trailing={
          <div className="flex items-center gap-3 text-[10px] font-mono text-zinc-600">
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded bg-sky-400/70" />Vol</span>
      ...
      <div className="px-3 py-2" style={{ height: chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 4, right: 20, left: 0, bottom: 4 }}>
            ...
            <Bar dataKey="vol" fill="#38bdf8" fillOpacity={0.7} radius={[4, 4, 4, 4]} barSize={10} />
...
function XaiNarratives({ xai }: { xai: Xai }) {
  const sections = [
    { label: '투자 스토리', content: xai.portfolioNarrative, icon: FileText },
        ...
        <ul className="space-y-2">
          {items.map((item, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className="text-xs font-mono text-emerald-500/70 mt-0.5 shrink-0 w-5 text-right tabular-nums">
        ...
        {hasVar && (
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={label} className="text-center p-3 rounded-lg bg-zinc-900/45 border border-zinc-800/70">
  ...
  return (
    <div className="space-y-2">
      {risk.anomalies.map((a, i) => {
        ...
        return (
          <div key={i} className={`flex items-center gap-2.5 px-4 py-3 rounded-lg border ${colorClass}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-zinc-950/40 border border-zinc-800/70 rounded-lg px-4 py-3">
      ...
      {data.portfolioThesis && (
        <Card>
          <CardHeader icon={FileText} label="포트폴리오 전략" />
          <div className="p-5">
      ...
      <Card>
        <CardHeader
          ...
          label="종목 배분"
          trailing={
            <span className="text-xs font-mono text-zinc-600 bg-zinc-900/70 px-2 py-0.5 rounded-md border border-zinc-800/70">
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          {data.allocations.length > 0 && (
            <div className="lg:w-[280px] shrink-0 p-4 border-b lg:border-b-0 lg:border-r border-zinc-800/70 flex flex-col items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            {/* Header */}
            <div className="px-5 py-2 bg-zinc-900/45 border-b border-zinc-800/70 flex items-center gap-3 text-[10px] font-mono text-zinc-600 uppercase">
